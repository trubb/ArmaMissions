waitUntil {!isNull player};

player createDiaryRecord ["Diary", ["Victory Conditions & Credits","
Victory Conditions:
<br/>The task is accomplished when:
<br/>The five fighting positions have been captured.
<br/>
<br/>If you take casualties exceeding 80% of your original force you are forced to retreat and the mission is deemed a failure.
<br/>
<br/>This mission was created for Anrop.se by Trubb
"]];

player createDiaryRecord ["Diary",["V. Command & Signal","
ALL FREQUENCIES ARE PREDEFINED, YOU ONLY NEED TO CHANGE CHANNEL!
<br/>
<br/>Company/Platoon net:
<br/>Platoon net - Channel 1
<br/>FO/Mortar net - Channel 2
<br/>
<br/>Squad Channels:
<br/>ALPHA QUEBEC - Channel 6
<br/>ECHO ALPHA - Channel 1
<br/>FOXTROT ALPHA - Channel 2
<br/>GOLF ALPHA - Channel 3
<br/>HOTEL ALPHA - Channel 4
<br/>KILO ALPHA - Channel 5
<br/>
<br/>Callsigns
<br/>ALPHA QUEBEC - Platoon lead
<br/>-   OSCAR ALPHA - Forward Observer
<br/>-   ECHO ALPHA - 1st squad
<br/>-   FOXTROT ALPHA - 2nd squad
<br/>-   GOLF ALPHA - 3rd squad
<br/>-   HOTEL ALPHA - 4th squad
<br/>-   KILO ALPHA - Mortar team
"]];

player createDiaryRecord ["Diary",["IV. Service & Support","
a. Equipment
<br/>Basic combat equipment.
<br/>Squad leaders have been assigned flare guns.
<br/>Additional ammunition is loaded in the platoon's Tgb 11.
<br/>
<br/>b. Medical resources
<br/>Advanced ACE medical. Your platoon medic can help you!
<br/>
<br/>No other help, you're on your own!
"]];

player createDiaryRecord ["Diary",["III. Mission","
Seize the five fighting positions in any order, and defeat the enemy's attempts at retaking them.
<br/>
<br/><marker name = '1'>Fighting position one</marker>
<br/><marker name = '2'>Fighting position two</marker>
<br/><marker name = '3'>Fighting position three</marker>
<br/><marker name = '4'>Fighting position four</marker>
<br/><marker name = '5'>Fighting position five</marker>
"]];

player createDiaryRecord ["Diary",["IIB. Friendly Forces","
You are one Swedish rifle platoon, roughly corresponding to the IB66 organisational model.
<br/>
<br/>No other friendly forces are present in the area.
"]];

player createDiaryRecord ["Diary",["IIA. Enemy Forces","
The Warszaw pact forces in the area consist of Polish naval infantry.
<br/>
<br/>Scouts report that roughly one company defend the fighting positions east of the village SUMMA.
<br/>
<br/>Expect reinforcements from the north to arrive quickly.
"]];

player createDiaryRecord ["Diary",["I. Situation","
Forces from the Warszaw pact has performed naval and airborne landings in southern Sweden.
<br/>They have forced back the swedes from the area around the village SUMMA.
<br/>
<br/>As part of a larger counter-attack your platoon has now been given the mission to retake a group of defensive positions.
"]];

player createDiaryRecord ["Diary",["IMPORTANT NOTES!","
A setup timer is present in the mission.
<br/>You should probably not cross into the combat zone while it is active.
<br/>
<br/>A hard completion timer is also present.
<br/>After crossing the red line you have 50 minutes to complete the mission.
<br/>
<br/>To make calculations easier the center point of the mortar squad is the mortar marker.
<br/>
<br/>Due to the era this mission is played in your force has no night vision devices or flashlights available.
<br/>Instead you will have to rely on your flare guns, or flares fired by the mortar team if you require additional lightning.
"]];
